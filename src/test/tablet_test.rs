...
pub fn assert_tablet_consistency(tablet: &TabletState) {
  ...
  for (query_id, ms_query_es) in &statuses.ms_query_ess {
    for child_qid in &ms_query_es.pending_queries {
      if let Some(es) = statuses.top.ms_table_read_ess.get(child_qid) {
        assert_eq!(&es.general.ms_query_id, query_id);
      } else if let Some(es) = statuses.top.ms_table_write_ess.get(child_qid) {
        assert_eq!(&es.general.ms_query_id, query_id);
      } else if let Some(es) = statuses.top.ms_table_insert_ess.get(child_qid) {
        assert_eq!(&es.general.ms_query_id, query_id);
      } else if let Some(es) = statuses.top.ms_table_delete_ess.get(child_qid) {
        assert_eq!(&es.general.ms_query_id, query_id);
      } else {
        panic!();
      }
  ...
  for (query_id, es) in &statuses.top.ms_table_read_ess {
  ...
  for (query_id, es) in &statuses.top.ms_table_write_ess {
    if let Some(ms_query_es) = statuses.ms_query_ess.get(&es.general.ms_query_id) {
      assert!(ms_query_es.pending_queries.contains(query_id));
    } else {
      panic!()
  ...
  for (query_id, es) in &statuses.top.ms_table_insert_ess {
    if let Some(ms_query_es) = statuses.ms_query_ess.get(&es.general.ms_query_id) {
      assert!(ms_query_es.pending_queries.contains(query_id));
    } else {
      panic!()
  ...
  for (query_id, es) in &statuses.top.ms_table_delete_ess {
    if let Some(ms_query_es) = statuses.ms_query_ess.get(&es.general.ms_query_id) {
      assert!(ms_query_es.pending_queries.contains(query_id));
    } else {
      panic!()
...
pub fn check_tablet_clean(tablet: &TabletState, check_ctx: &mut CheckCtx) {
  ...
  check_ctx.check(statuses.top.trans_table_read_ess.is_empty());
  check_ctx.check(statuses.top.ms_table_read_ess.is_empty());
  check_ctx.check(statuses.top.ms_table_write_ess.is_empty());
  check_ctx.check(statuses.top.ms_table_insert_ess.is_empty());
  check_ctx.check(statuses.top.ms_table_delete_ess.is_empty());
